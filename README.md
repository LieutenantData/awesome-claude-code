...
- [ralph-learns-marathon](https://github.com/LieutenantData/ralph-learns-marathon) by
  [LieutenantData](https://github.com/LieutenantData) - Sprint-based backlog management for Ralph that solves the context-window problem with large PRDs (100-500+ stories). Splits a full backlog into compact, module-sized sprint files (~40 KB) that fit comfortably in an AI agent's context window. Includes automatic progress syncing across sprints, a markdown-to-backlog converter, and per-module progress tracking. Pure Python, no jq dependency.
